...
if (gadgetHandler:IsSyncedCode()) then
  ...
  function gadget:FeatureCreated(FeatureID,allyTeam)
    ...
    SendToUnsynced("unitshaders_FeatureCreated", FeatureID, allyTeam)
  end

  function gadget:FeatureDestroyed(unitID,allyTeam)
    SendToUnsynced("unitshaders_FeatureDestroyed", unitID, allyTeam)
  end
...
function gadget:Initialize()
  ...
  do
    ...
    local FMATERIALS_DIR = GADGET_DIR .. "FeatureMaterials/"
...
do
    local MATERIALS_DIR = GADGET_DIR .. "FeatureMaterials/"

    local files = VFS.DirList(FMATERIALS_DIR)
    table.sort(files)

    for i=1,#files do
      local fmats, fTexture = VFS.Include(files[i])
      tmerge(fmaterialDefs, fmats)
      tmerge(featureMaterialDefs, fTexture)
...
--// process the feature materials (compile shaders, load textures, ...)
  do
    for _,mat_src in pairs(fmaterialDefs) do
      if (mat_src.shader)and
         (mat_src.shader ~= "3do")and(mat_src.shader ~= "s3o")
      then
        mat_src.shaderSource = mat_src.shader
    ...
    for featureName,materialInfo in pairs(featureMaterialDefs) do
      if (type(materialInfo) ~= "table") then
        materialInfo = {materialInfo}
      end
  ...
  gadgetHandler:AddSyncAction("unitshaders_FeatureCreated", FeatureCreated)
  gadgetHandler:AddSyncAction("unitshaders_FeatureDestroyed", FeatureDestroyed)
