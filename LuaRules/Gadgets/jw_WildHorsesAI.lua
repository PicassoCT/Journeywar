...
if (gadgetHandler:IsSyncedCode()) then
	...
	--kdronewarrior kdronestructure contruck kairdronefactory kdroneminer
	...
	function doesUnitWantBuildJob (unitID, childname)
		...
		if (unitName (unitID) == "mdigg") then
			if (#Spring.GetTeamUnitsByDefs (Spring.GetUnitTeam (unitID), {UnitDefNames["mdigg"].id}) > 0) then return false end
		end
	...
	name=unitName (unitID)
		if ( name == "conair" or unit) then supportBuild (unitID, "conair") end
	
		
		if (buildUnitOrder[name] and  buildUnitOrder[name]==true ) then --land factory builds
		...
		if (not unitOnMission[unitID] and name == "mdigg" and jobname == "mdiggmex") then
		...
		if (name == "citadell") then --engineer building whatever building
	...
	--returns how many contruck and kdroneminingtowers a team has
	function countTowersAndEngineer (teamID)
		return #Spring.GetTeamUnitsByDefs (teamID, {UnitDefNames["mdiggmex"].id}) + Spring.GetTeamUnitsByDefs (teamID, {UnitDefNames["contruck"].id})
	end
	...
	function gadget:GameFrame(frame)
		...
		for _,t in pairs(myTeam) do
			...
			if (missing) then
				if (unitCount (myTeam[t], "contruck") < (missing["citadell"] or 0) ) then
					missing["citadell"] = (missing["citadell"] or 0) + (missing["contruck"] -unitCount (myTeam[t], "citadell"))
				end
				makeSomeUnits (myTeam[t], missing)
				supportBuildEmptyMiningTowers (myTeam[t])
	...
	function machTargetArea (teamID, x, z) --alle rushen mit
		...
		if (all_units == nil) then return end		
		for i,unitID in pairs(all_units) do
			if ( ( unitName (unitID) ~= "contrain" ) and (Spring.ValidUnitID (unitID)) ) then
				Spring.GiveOrderToUnit(unitID, CMD.GUARD , {x, Spring.GetGroundHeight (x,z), z }, {})
			end
		end
	end
	...
	function deployNearRes (unitID, keys)
		...
		--local ux,uy,uz = Spring.GetUnitPosition (unitID)
		local x,y,z = getBuildSpot (rx,ry,rz, "mdigg", 20, 5)
	...
	function deployAt (unitID, tx,tz, keys)
		...
		Spring.GiveOrderToUnit(unitID, 31210,{UnitDefNames["mdigg"].id},{"shift"})
	end
	
	function supportBuild (unitID,name)
		
		Spring.GiveOrderToUnit(unitID, 31210,{UnitDefNames[name].id},{})
	end
	...
	function supportBuildRandomMiningTowers (teamID, n)
	end
	---------------------------------
	
	----------TICK TACK LOGIC (gets executed check every nth frame)------
	function supportBuildEmptyMiningTowers (teamID)
		local mtowers = Spring.GetTeamUnitsByDefs (teamID, {UnitDefNames["mdigg"].id})
		...
		for i in pairs (mtowers) do
			...
			if ((nearest_res_distance or 0) > 150) then supportBuild (mtowers[i], "contruck") end
		end
	end
	
	--[[
	function sendOutIdleEngineers (teamID)
		local engs = Spring.GetTeamUnitsByDefs (teamID, {UnitDefNames["contruck"].id})
	...
	function clonetest (teamID)
		local munits = Spring.GetTeamUnitsByDefs (teamID, {UnitDefNames["contruck"].id})
		for i in pairs (munits) do
			cloneUnit (munits[i], UnitDefNames["contruck"].id)
		end
	end
	------------------------------------------------------
	...
	function gadget:UnitFinished(unitID, unitDefID, teamID)	
		...
		if (isTeamCBM (teamID)) then
			...
			--end
			if ((unitName (unitID) ~= "contruck") or (unitName (unitID) ~= "mtw")) then
				local x = math.random(Game.mapSizeX)
				local z = math.random(Game.mapSizeZ)
				Spring.GiveOrderToUnit(unitID, CMD.FIGHT , {x, Spring.GetGroundHeight (x,z), z }, {})
			end
			if (unitName (unitID) == "mtw") then 
				machPatrol(teamID)
			end
		else
