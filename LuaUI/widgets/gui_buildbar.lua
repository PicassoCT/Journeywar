...
local sound_waypoint  = LUAUI_DIRNAME .. 'Sounds/buildbar/buildbar_waypoint.wav'
local sound_click     = LUAUI_DIRNAME .. 'Sounds/buildbar/buildbar_click.WAV'
...
-------------------------------------------------------------------------------
local iconSizeX  = bar_iconSizeBase
...
local function DrawTexRect(rect, texture, alpha)
  ...
  glTexRect(rect[1],rect[2],rect[3],rect[4])
...
local function DrawButton(rect, unitDefID, options)
  ...
  if (options.progress or -1)>-1 then
    DrawBuildProgress(rect[1],rect[2],rect[3],rect[4], options.progress, { 0, 0.9, 1, 0.5 })
  end
