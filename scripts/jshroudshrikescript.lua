...
function buildAtree(id)

if it <1 or it > NUMBEROFPIECES then return end
...
contOrFork=it
	 ...
	 if immunity < 0 or (math.random(0,1)==1  and count %4 ==0) then
	 immunity=4
	 contOrFork= math.min(it+ immunity,NUMBEROFPIECES)
	 end
	...
	ex,ey,ez=Spring.GetUnitPiecePosDir(unitID,Endpiece[contOrFork])
	
	Move(TreePiece[it],x_axis,ex-x,0)
	Move(TreePiece[it],y_axis,ey-y,0)
	Move(TreePiece[it],z_axis,ez-z,0,true)

		v=math.random(-95.4,92.4)
	Turn(TreePiece[it],x_axis,math.rad(v),0)
		v=math.random(-95.4,92.4)
	Turn(TreePiece[it],y_axis,math.rad(v),0)
		v=math.random(-95.4,92.4)
	Turn(TreePiece[it],z_axis,math.rad(v),0)



x,y,z=Spring.GetUnitPiecePosition(unitID,Endpiece[math.min(NUMBEROFPIECES,it+1)])
...
it=it+1
	if id then updateUnitsPosition() end
	
end

function buildCtree(id)

if it <1 or it > NUMBEROFPIECES then return end
count=count+1
contOrFork=it
		
	 if math.random(0,1)==1  and count %2 ==0 then
	 contOrFork= math.ceil(math.random(up,it))
	 up=math.min(up+up/count,it-1)
	 end
	 
	x,y,z=Spring.GetUnitPosition(unitID)
	
	ex,ey,ez=Spring.GetUnitPiecePosDir(unitID,Endpiece[math.max(1,contOrFork-1)])
	...
	Turn(TreePiece[it],x_axis,math.rad(math.random(-4.4,4.4)*10),0)
	Turn(TreePiece[it],y_axis,math.rad(math.random(-4.4,4.4)*10),0)
	Turn(TreePiece[it],z_axis,math.rad(math.random(-4.4,4.4)*10),0,true)
...
contOrFork=it
	...
	Move(TreePiece[it],z_axis,ez-z,0,true)
		...
		assert(TreePiece[it])
	Turn(TreePiece[it],x_axis,math.rad(math.random(-2.4,2.4)*10),0)
	Turn(TreePiece[it],y_axis,math.rad(math.random(-2.4,2.4)*10),0)
	Turn(TreePiece[it],z_axis,math.rad(math.random(-2.4,2.4)*10),0,true)
