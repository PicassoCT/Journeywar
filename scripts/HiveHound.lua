...
deathpivot=piece"deathpivot"
	...
	local isPieceInTurn= Spring.UnitScript.IsInTurn
		...
		while(true) do
			for i=1,3,1 do --itterate over the Pack
...
if math.abs(degree) > 45 then return (degree/math.abs(degree))*45 else return degree end
    ...
    function clampZ(nrOfTit,degree)
		if nrOfTit <4 or (nrOfTit >6 and nrOfTit < 10) then-- (obere Reihe)
			if nrOfTit > 4 then 
				if degree > 67  then return 66 elseif degree < -33 then return -33 else return degree end
			else
				if degree < -67 then return -66 elseif  degree > 33 then return 33 else return degree end
			end
		else
			if nrOfTit < 7 then 
				if degree > 19 then return 19 elseif  degree < -42 then return -42 else return degree end
			else
				if degree > -19 then return -19 elseif  degree < 42 then return 42 else return degree end
			end
